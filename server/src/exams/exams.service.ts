...
export class ExamsService {
  ...
  async getStudentStats(userId: string) {
    const attempts = await this.prisma.examAttempt.findMany({
      where: { userId, status: { in: [AttemptStatus.SUBMITTED, AttemptStatus.GRADED] } },
      include: { exam: true },
    });
  ...
  async getLecturerStats(userId: string) {
    ...
    const exams = await this.prisma.exam.findMany({
      ...
      include: { 
        _count: { 
          select: { 
            attempts: { where: { status: AttemptStatus.SUBMITTED } } 
          } 
        } 
      }
    });
  ...
  async getAdminStats() {
    const [totalUsers, totalExams, pendingApprovals, activeNow] = await Promise.all([
      ...
      this.prisma.examAttempt.count({ where: { status: AttemptStatus.STARTED } }),
    ]);
